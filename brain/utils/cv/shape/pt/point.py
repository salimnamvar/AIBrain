...
from brain.utils.misc import is_int, is_float
...
class Point2D(BaseObject):
    ...
    def __init__(
        self, a_x: Union[int, float], a_y: Union[int, float], a_name: str = "POINT"
    ) -> None:
    ...
    def validate_array(
        ...
        if a_coordinates.shape[-1] < 2:
            raise ValueError(
                f"`a_coordinates` array should at least have length 2 but it is in shape of"
    ...
    def speed_xy(self, a_point2d: "Point2D") -> Tuple[float, float]:
        ...
        norm = (
            np.sqrt((a_point2d.y - self.y) ** 2 + (a_point2d.x - self.x) ** 2) + 1e-10
        )
...
class Point2DList(BaseObjectList[Point2D]):
    ...
    def from_xy(
        ...
        points.append(
            a_item=[
                Point2D.from_xy(a_coordinates=coord, **kwargs) for coord in coordinates
            ]
        )
        return points
