...
from brain.utils.cv.shape.bx import BBox2DList, BBox2D
from brain.utils.cv.vid import Frame2D
from brain.utils.ml.trk import TrackedBBox2DDict, TrackedBBox2D
...
class OCSORT(BaseModel):
    ...
    def __init__(
        ...
        # Create Population for each pop's ID
    ...
    def tracked_population(self, a_tracked_population: PopulationDict) -> None:
        ...
        """
        if a_tracked_population is None or not isinstance(
            a_tracked_population, PopulationDict
        ):
    ...
    def _preproc(self, a_boxes: BBox2DList, a_frame: Frame2D) -> tuple:
        ...
        inds_second = np.logical_and(
            inds_low, inds_high
        )  # self._det_thresh > score > 0.1, for second matching
        ...
        velocities = np.array(
            [
                (
                    trk.statistics.velocity
                    if trk.statistics.velocity is not None
                    else np.array((0, 0))
                )
        ...
        for trk in self.tracked_population[a_frame.video_id]:
            tmp.append(
                k_previous_obs(
                    trk.states, trk.statistics.prediction_age, self.delta_time
                )
            )
    ...
    def _update_matched_targets(
        ...
        for m in a_matched_pairs:
            ...
            box = BBox2D.from_xyxys(
                a_coordinates=a_dets[det_idx, :], a_do_validate=False
            )
        ...
        if (
            ...
            if iou_left.max() > self.iou_thre:
                ...
                for m in matched_indices:
                    ...
                    box = BBox2D.from_xyxys(
                        a_coordinates=a_dets_second[det_ind, :], a_do_validate=False
                    )
                    # Update target
        ...
        if a_unmatched_dets_idx.shape[0] > 0 and a_unmatched_targets_idx.shape[0] > 0:
            ...
            if iou_left.max() > self.iou_thre:
                ...
                for m in rematched_indices:
                    ...
                    box = BBox2D.from_xyxys(
                        a_coordinates=a_dets[det_ind, :], a_do_validate=False
                    )
    ...
    def _update_unmatched_targets(
        self, a_unmatched_targets_idx: np.ndarray, a_frame: Frame2D
    ) -> None:
